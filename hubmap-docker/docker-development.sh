...
function export_version() {
    export UKV_API_VERSION=$(tr -d "\n\r" < ../VERSION | xargs)
    echo "UKV_API_VERSION: $UKV_API_VERSION"
}
...
if [[ "$1" != "check" && "$1" != "config" && "$1" != "build" && "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "check" ]; then
        ...
        absent_or_newer ukv-api/src ../src

        echo 'Checks complete, all good :)'
    elif [ "$1" = "config" ]; then
        docker compose -f docker-compose.yml -f docker-compose.development.yml -p ukv-api config
    elif [ "$1" = "build" ]; then
        # Delete the copied source code dir if exists
        if [ -d "ukv-api/src" ]; then
            rm -rf ukv-api/src
        fi
        ...
        if [ -f "ukv-api/VERSION" ]; then
            rm -rf ukv-api/VERSION
        fi
        
        if [ -f "ukv-api/BUILD" ]; then
            rm -rf ukv-api/BUILD
        fi
        ...
        docker compose -f docker-compose.yml -f docker-compose.development.yml -p ukv-api stop
    elif [ "$1" = "down" ]; then
        docker compose -f docker-compose.yml -f docker-compose.development.yml -p ukv-api down
    fi
fi
